...
class Decoder(nn.Module):
  ...
  def forward(self, x, encoder_states, hidden, cell):
    ...
    attention_score_bahdanau = self.V(attention_score)
...
#     context_vector = torch.sum(context_vector, dim=1)
    ...
    # ===========================Luong====================================
    encoder_states_transposed = torch.transpose(encoder_states, 0, 1).permute(1, 2, 0)
    attention_score_luong = torch.matmul(hidden_with_time_axis, encoder_states_transposed).permute(0, 2, 1)
#     attention_weights_luong = torch.softmax(attention_score)
    
    # ===========================Concat====================================
    attention_weights = attention_score_bahdanau + attention_score_luong
    attention_weights_relu = self.relu(attention_weights)
    intermediate_outputs.append(attention_weights_relu)
    ...
    return predictions, hidden, cell, attention_weights, intermediate_outputs
